module.exports = {
    ...
    async execute(message, utils) {
        ...
        if (message.query.voice === data[0].voice) return message.reply(`the TTS voice is already set to: **${newVoice}** (${data[0].flag} ${data[0].language})`)

        await utils.query(`UPDATE guilds SET voice=? WHERE guild_id=?`, [voice, message.guild.id])
        utils.cache.del(message.guild.id)

        message.reply(`the Polly TTS voice has been successfully updated to \`${newVoice}\``)
    },
};