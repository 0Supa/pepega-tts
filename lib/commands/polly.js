...
const queue = require('../utils/ttsQueue.js')
...
module.exports = {
    ...
    async execute(message) {
        ...
        try {
            if (connection) {
                queue.add(phrase, message.query.voice, 'polly')
                const dispatcher = connection.play((await queue.getUrl()))

                dispatcher.on('end', async () => {
                    connection.play((await queue.getUrl()))
                })
